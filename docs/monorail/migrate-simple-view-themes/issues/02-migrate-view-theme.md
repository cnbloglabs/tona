...
- [ ] `themes/view/` 完整结构就位（package.json + vite.config.ts + src/）
- [ ] main.js 用 `import.meta.glob` 注册模块 + 13 个插件 `.use()` 链
- [ ] `modules/header/` `install()` 就位，6 个自研函数逻辑忠实迁移
- [ ] catalog 用新插件，配置映射到 `mountedNode`/`fn`/`updateNavigation`/`showTitle`/`showScrollbar`
- [ ] 样式迁移完整，plugins.scss 的 `@use ... with (...)` 映射正确（含注释掉的 copyCode/postSignature 接回），Font Awesome 保留
- [ ] `pnpm --filter tona-theme-view build` 构建成功，产物 `dist/view.js` 存在
- [ ] 产物含 `custom-searchbar`、`message-count`、`custom-gtihub`、`navbarBurger` 等关键字符串
- [ ] inline CSS 含 view 关键样式（`#q`/`.custom-searchbar`、`.message-count`、`#custom-gtihub` 等）
